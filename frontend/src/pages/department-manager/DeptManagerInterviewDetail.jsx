...
export default function DeptManagerInterviewDetail() {
  ...
  const loadInterview = async () => {
    try {
      ...
      if (!data) {
        notify.error("Không tìm thấy phỏng vấn hoặc bạn không có quyền xem");
  ...
  const readOnlyNominator =
    !!(interview.isReadOnlyNominatorAccess ?? interview.IsReadOnlyNominatorAccess);
  ...
  const participationPending = !readOnlyNominator && !myConfirmed && !myDeclined;
    ...
    if (Number.isNaN(d.getTime())) {
      ...
      if (/^\d{4}-\d{2}-\d{2}/.test(iso)) {
        ...
        if (!Number.isNaN(d2.getTime()))
          return d2.toLocaleDateString("vi-VN", { month: "2-digit", year: "numeric" });
      }
      return "";
    }
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-0.5 text-sm font-medium text-slate-800">
            {candidate.yearsOfExperience != null && candidate.yearsOfExperience !== ""
            ...
            {(candidateCertificates || []).map((c, idx) => (
              <div key={idx} className="rounded-lg border border-slate-200 bg-white p-3">
                ...
                <div className="text-xs text-slate-600">
                  {[c.issuer || c.Issuer, (c.issuedYear ?? c.IssuedYear) ? `Năm ${c.issuedYear ?? c.IssuedYear}` : null]
                    .filter(Boolean)
                    .join(" • ") || "—"}
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-5xl">
        ...
        {readOnlyNominator ? (
          <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950 shadow-sm">
            <span className="font-semibold">Chế độ chỉ xem:</span> Bạn đã đề cử người tham gia buổi này và{" "}
            <span className="font-semibold">không tham gia với tư cách interviewer</span>. Bạn có thể xem thông tin & hồ
            sơ ứng viên; không thể xác nhận tham gia hay nộp đánh giá tại đây.
          </div>
        ) : null}

        {!readOnlyNominator ? <div className="mb-4">{participationSection}</div> : null}
