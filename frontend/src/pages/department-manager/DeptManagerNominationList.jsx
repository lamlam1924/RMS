      ...
      {pendingReqs.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {pendingReqs.map(req => (
              <div key={req.id} style={{ backgroundColor: 'white', borderRadius: 8, border: '1px solid #fde68a', padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    {req.message && (
                      <div style={{ fontSize: 13, color: '#374151', marginTop: 6, fontStyle: 'italic', backgroundColor: '#fffbeb', padding: '6px 10px', borderRadius: 6 }}>
                        💬 {req.message}
                      </div>
                    )}
                  </div>
                  <button
                    ...
                    style={{ padding: '8px 16px', backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 600, fontSize: 13, whiteSpace: 'nowrap' }}
                  >
                    Đề cử người
                  </button>
                </div>
                ...
                {activeReqId === req.id && (
                  <div style={{ marginTop: 16, borderTop: '1px solid #e5e7eb', paddingTop: 16 }}>
                    <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 10 }}>
                      Chọn thành viên tham gia ({selectedIds.length}/{req.requiredCount} người)
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                      <button onClick={() => setActiveReqId(null)} style={{ padding: '8px 16px', border: '1px solid #d1d5db', borderRadius: 6, backgroundColor: 'white', cursor: 'pointer' }}>
                        Hủy
                      </button>
                      <button
                        onClick={handleNominate}
                        disabled={submitting || !selectedIds.length}
                        style={{ padding: '8px 16px', backgroundColor: '#10b981', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 600, opacity: (submitting || !selectedIds.length) ? 0.7 : 1 }}
                      >
