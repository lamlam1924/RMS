...
/**
 * Trang xác nhận / từ chối tham gia phỏng vấn (interviewer — mở từ link trong email).
 * Route: /staff/employee/interviews/:id/confirm hoặc .../decline
 */
export default function EmployeeInterviewConfirm() {
  const { id } = useParams();
        ...
        {!done ? (
          <>
            ...
            <p style={{ fontSize: 14, color: '#6b7280', marginBottom: 24 }}>
              {isDecline
                ? 'Bạn chắc chắn muốn từ chối lịch phỏng vấn này? Bạn có thể ghi chú lý do hoặc đề xuất khung giờ khác để HR/Trưởng phòng xem xét đổi lịch.'
                : 'Bấm bên dưới để xác nhận bạn sẽ tham gia buổi phỏng vấn.'}
            </p>
            {isDecline && (
              <div style={{ marginBottom: 20, textAlign: 'left' }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>
                  Ghi chú cho HR/Trưởng phòng (tùy chọn)
                </label>
                <textarea
                  rows={3}
                  value={note}
